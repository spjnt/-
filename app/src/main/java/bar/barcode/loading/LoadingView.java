...
public class LoadingView extends LinearLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        ltv = findViewById(R.id.loading_text);

        ViewHelper.setScaleX(mIndicationIm, 0.2f);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LoadingView);
        String loadText = typedArray.getString(R.styleable.LoadingView_load_Text);
        int textAppearance = typedArray.getResourceId(R.styleable.LoadingView_load_Text, -1);
        ...
        setLoadingText(loadText, R.color.white);
    }
    ...
    public void setLoadingText(CharSequence loadingText, int colorID) {
        if (TextUtils.isEmpty(loadingText)) {
            mLoadTextView.setVisibility(GONE);
            mLoadTextView.setTextColor(getResources().getColor(colorID));
            ltv.setTextColor(getResources().getColor(colorID));
            ltv.setVisibility(GONE);
        } else {
            mLoadTextView.setVisibility(VISIBLE);
            mLoadTextView.setTextColor(getResources().getColor(colorID));
            ltv.setTextColor(getResources().getColor(colorID));
            ltv.setVisibility(VISIBLE);
            ltv.start();
        }
    ...
    public void setLoadingText(int loadingText, int colorID) {
        int color = getResources().getColor(colorID);
        String string = getResources().getString(loadingText);
        if (TextUtils.isEmpty(string)) {
            mLoadTextView.setVisibility(GONE);
            mLoadTextView.setTextColor(color);
            ltv.setTextColor(color);
            ltv.setVisibility(GONE);
        } else {
            mLoadTextView.setVisibility(VISIBLE);
            mLoadTextView.setTextColor(color);
